  ...
  describe "submit_shipping_label_request" do
    ...
    describe "create_shipping_labels" do
      ...
      it "executes create_shipping_labels request" do
        ...
        response = vendor_direct_fulfillment_shipping_client.create_shipping_labels(purchase_order_number, selling_party, ship_from_party)
        expect(response.response_code).to eq(200)
        expect(JSON.parse(response.body)).to include(
          "purchaseOrderNumber" => purchase_order_number,
      ...
      describe "get_customer_invoices" do
        it "executes get_customer_invoices request" do
          ...
          response = vendor_direct_fulfillment_shipping_client.get_customer_invoices(created_after, created_before, params)
          expect(response.response_code).to eq(200)
          expect(JSON.parse(response.body)).to include(
            "pagination" => {
              "nextToken" => be_present
