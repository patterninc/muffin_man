...
RSpec.describe MuffinMan::FulfillmentInbound::V20240320 do
  ...
  describe "get_shipment_items" do
    ...
    before do
      stub_get_shipment_items(inbound_plan_id, shipment_id, page_size: page_size, pagination_token: pagination_token)
    end

    it "gets shipment items" do
      response = fba_inbound_client.get_shipment_items(inbound_plan_id, shipment_id, page_size: page_size,
                                                                                     pagination_token: pagination_token)
      expect(response.success?).to be true
      expect(JSON.parse(response.body)["items"]).to be_an(Array)
    end
